...
The following figures provide a side-by-side look at how average calories, fat, and protein levels differ across major fast-food chains. By comparing these three metrics, we can better understand the nutritional “profiles” of each restaurant by highlighting ones that may be better suited for energy-dense meals, lean eating, or balanced options. Bar charts (Figures 2–4) compare each metric:

-   Figure 2: Avg Calories
-   Figure 3: Avg Fat (g)
-   Figure 4: Avg Protein (g)

**Figure 2** displays the average calorie content of menu items by restaurant. This metric gives a general sense of how energy-dense the offerings are at each chain. Restaurants with higher average calories often serve larger portions or more indulgent items, such as burgers, fried foods, or combo meals. For fitness-focused individuals watching their caloric intake, this figure provides a helpful baseline for identifying chains where portion control or selective ordering may be especially important.
...
Calorie content alone doesn’t tell the whole story, but it establishes a key constraint, especially for people that are managing their weight or following specific intake goals. Later figures help put this calorie context into perspective by comparing it with fat and protein levels.

**Figure 3** illustrates the average fat content per menu item by restaurant. While fat is a necessary nutrient, excessive intake, like from saturated/trans fats, has been associated with increased health risks such as cardiovascular disease. According to the Harvard T.H. Chan School of Public Health, not all fats are created equal; distinguishing between healthy unsaturated fats and harmful trans fats is critical to informed dietary choices. Fast-food chains that rely heavily on frying, cheese, or processed meats tend to rank higher in fat content. Understanding these differences can help consumers better regulate their fat intake while still enjoying convenient meals.
...
When viewed alongside calories and protein, fat content provides an important middle layer. Some high-calorie items may be energy-dense due to fat, while others may balance it out with higher protein, which emphasizes the need to look at nutrients together rather than separately.

**Figure 4** revisits protein content, this time as a standalone measure rather than in comparison to fat or calories. A high-protein diet plays a vital role in muscle synthesis, post-exercise recovery, and maintaining lean body mass. As Phillips and van Loon explain, dietary protein supports optimal adaptations to strength training and contributes to satiety, making it a critical macronutrient for active individuals (*Journal of Sports Sciences*). This figure helps readers identify restaurants that offer consistently higher protein content, which aligns with recommendations for physically active populations. For instance, restaurants like Chipotle and Subway stand out, offering options that may better meet these kinds of nutritional demands.
...
Comparing Figure 4 with the previous plots reinforces the idea that not all high-calorie or high-fat meals are bad if they also deliver strong protein benefits. This sets the stage for analyzing combined metrics, like the protein-to-fat ratio, in later sections.

**Table 1** provides the average calorie, fat, and protein content of menu items for each restaurant in the dataset. Unlike bar charts, which are better for visual trend-spotting, this summary table offers precise values that allow for closer comparison between chains.
...
**Figure 5** revisits average protein content per restaurant, now with item counts to show how many menu items were analyzed per chain. This added detail is important as averages based on very few items (like a single high-protein bowl) may be less representative than ones from chains with larger, more diverse menus.

The chart also helps identify not only which restaurants offer higher-protein meals on average, but also whether those averages are backed by a wider variety of choices. For instance, a chain with both a higher average and a larger number of items (n) may be more reliably protein-focused overall. In contrast, a restaurant with a high average but low item count may only offer a few protein-heavy options which can be potentially limiting for consumers trying to make consistent nutritional choices.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
library(tidyverse)

#3.Average Protein per Menu Item by Restaurant
# Calculate average protein and item count per restaurant
protein_stats <- data %>%
  group_by(Restaurant) %>%
  summarize(
    Avg_Protein = mean(Protein, na.rm = TRUE),
  ...
  mutate(Avg_Protein = ifelse(is.na(Avg_Protein), 0, Avg_Protein)) #Replace null values with 0

#Bar graph with count labels (include how many items per restaurant)
ggplot(protein_stats, aes(x = reorder(Restaurant, Avg_Protein), y = Avg_Protein)) +
  geom_col(fill = "#4682B4") +
  geom_text(aes(label = paste0("n=", Item_Count)), hjust = 1.1, color = "white", size = 4) +
  coord_flip() +
  labs(
...
In summary, Figure 5 reinforces the importance of not just looking at average protein levels, but also considering how many options a restaurant offers to support consistent dietary habits. Chains with both high protein averages and a large number of items, like Chipotle, may offer more flexibility for fitness-focused consumers. This layered view lays the groundwork for deeper comparisons, such as evaluating how efficiently each restaurant delivers protein relative to fat and calories.
...
#4. Ranking Protein-Fat Ratios Per Restaurants
#Find the protein-to-fat (PF) ratio
data <- data %>%
  mutate(PF_Ratio = Protein / Fat)

#Get top 3 items per restaurant based on PF ratio
top3_pf_items <- data %>%
...
#Display results with labelled columns
top3_pf_items %>%
...
#Single-best item per restaurant based on PF ratio
best_pf_item <- data %>%
  filter(Fat > 0, !is.na(Protein), !is.na(Fat)) %>%
  group_by(Restaurant) %>%
